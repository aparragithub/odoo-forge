...
``validate_repository`` (validate.py:597) calls ``validate_documentation(root)``
by module-global name lookup at call time, so patching the ``validate`` module
attribute reaches every caller that goes through ``validate_repository``. This
autouse fixture replaces ``validate.validate_documentation`` with a thin
wrapper around the *original* function whose ``run_renderer`` default is a
no-op returning ``RendererResult(0, "")`` instead of invoking
``run_fixed_renderer`` (which shells out to the Docker-backed
...
at def time (validate.py:305, validate.py:461), not looked up by name at call
time, so they are NOT patchable from outside the module. Patching
``validate.validate_documentation`` at the module-attribute level is the only
external seam available without editing ``validate.py``.
