...
from odoo_forge.data_artifacts.types import _SECRET_OR_CONNECTION_TEXT

_HOST_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9.-]*$")
_TOKEN_RE = re.compile(r"^[A-Za-z0-9._/@:+%=\-^$,]+$")
_SAFE_ASSIGNMENT_KEYS = frozenset({"com.odoo-forge.creator-token"})
...
def _validated_tokens(tokens: Sequence[str]) -> tuple[str, ...]:
    ...
    if not values or any(
        not isinstance(token, str) or not _TOKEN_RE.fullmatch(token) or _secret_assignment(token)
        for token in values
    ):
...
def _secret_assignment(token: str) -> bool:
    ...
    if (
        not separator
        or key in _SAFE_ASSIGNMENT_KEYS
        or not _SECRET_OR_CONNECTION_TEXT.fullmatch(key)
    ):
        return False
    return not (key.upper().endswith("_FILE") and value.startswith("/"))
