...
- [x] 5a.1 RED: run `uv run pytest tests/test_validate.py tests/test_onboard.py -v` pre-move (baseline). The move was already applied to the working tree (uncommitted) at the start of this batch; baseline was re-established by confirming the current 27 tests pass post-move (`tests/cli/test_validate.py` + `tests/cli/test_onboard.py`), then proving the repoint is load-bearing via the adversarial wrong-target check in 5a.4.
- [x] 5a.2 Create `src/odoo_forge_cli/commands/manifest.py`; move `validate`, `onboard`; add `register(app)` (extended in PR5b). Confirmed: 178 lines, imports `_composition`/`_presentation`/`_support` module-qualified plus `enterprise_credential` symbols bare (mirrors pre-move `main.py` shape); no import of `odoo_forge_cli.main`.
- [x] 5a.3 In `main.py`, import `commands.manifest` and call `register(app)`; remove `validate`/`onboard` bodies. Confirmed: `from odoo_forge_cli.commands import backend, image, maintenance, manifest` + `manifest.register(app)`; `main.py` now 190 lines with 3 `@app.command` decorators (lock/project/unlock).
- [x] 5a.4 Repoint `tests/cli/test_validate.py`: already satisfied ahead of schedule by PR1's wide qualification (task 1.8) — `_support._resolve_mount_base` / `_composition._make_workspace_provider` (underscored spelling, per PR1's documented naming decision), not `main.*`. Verified by `rg` (no `main.` references) and by the adversarial check: retargeting `test_onboard.py`'s `_composition._make_workspace_provider` patch to `_support` instead raised `AttributeError: <module ... _support> has no attribute '_make_workspace_provider'` (11 failures), then restored — confirming the real target genuinely binds the call site.
- [x] 5a.5 Repoint `tests/cli/test_onboard.py`: same as above, already satisfied by PR1 (`_support._resolve_mount_base` direct call at former l.45, `_composition._make_workspace_provider` monkeypatch). Additionally discovered and fixed a PR5a-caused regression NOT listed in the original task: `tests/cli/test_enterprise_credential.py` patched `main._make_enterprise_credential_resolver` for 4 onboard-invoking tests, which silently stopped intercepting once `onboard` moved to `commands/manifest.py` (which bare-imports `_make_enterprise_credential_resolver`, so the call site now resolves against `commands.manifest`'s own globals — the same bare-import pattern PR4 established for `plan_backend`). Repointed those 4 tests' onboard-path patches to `commands.manifest._make_enterprise_credential_resolver` (added `from odoo_forge_cli.commands import manifest` import); the 5th affected test (`test_lock_and_onboard_fail_identically_on_missing_credential`) invokes both commands in one test, so it now patches BOTH `main` (for `lock`, still in `main.py`) AND `manifest` (for `onboard`). Lock-only tests were left untouched (`lock` stays on `main`'s bare import until PR5b).
- [x] 5a.6 GREEN: `uv run pytest` full suite green — 901 passed, 17 deselected (identical to PR4's post-verification count; the one failure found during verification, `test_enterprise_credential_resolved_exactly_once_for_onboard`, was fixed by 5a.5's additional repoint, not a pre-existing regression).
- [x] 5a.7 Verify: `forge validate/onboard --help` byte-identical (diffed against `git stash`-restored PR4 baseline output, zero diff both); `uv run lint-imports` passes (109 files, 361 dependencies, 6/6 contracts kept); no cycle (`import odoo_forge_cli.commands.manifest; import odoo_forge_cli.main` succeeds); `ruff check`/`ruff format --check` clean.

## PR5b: `commands/manifest.py` — lock/unlock/project + thin `main.py`

- [x] 5b.1 RED: run `uv run pytest tests/test_lock.py tests/test_unlock.py tests/test_project.py tests/test_enterprise_credential.py -v` pre-move (baseline). Result: 57 passed (pre-move, PR5a state).
- [x] 5b.2 Move `lock`, `project`, `unlock` into `commands/manifest.py`; extend `register(app)` to bind all five manifest commands.
- [x] 5b.3 In `main.py`, remove `lock`/`project`/`unlock` bodies; drop the `__all__` re-export block (890-896) — no facade. Confirmed: `main.py` no longer imports/exposes `_bind_*`/`_make_enterprise_credential_resolver`/`_preflight_*` at all — those direct-call test sites were repointed to `ec.*` (task 5b.7) instead of kept as a facade.
- [x] 5b.4 Repoint `tests/test_lock.py`: `main._make_provider`/`main._make_published_artifact_resolver` were already `_composition.*` (satisfied by PR1, verified unchanged); the autouse `_make_enterprise_credential_resolver` patch → repointed `main` → `commands.manifest` (bare-imported in `commands/manifest.py`, so that module is now the one real call-site object); direct `main._load_lock` calls (l.297, 311) already repointed to `_support._load_lock` in PR1 — confirmed no regression (57 passed unchanged).
- [x] 5b.5 `tests/test_project.py`: already fully satisfied by PR1's wide qualification (task 1.8) — `_composition._make_workspace_provider`/`_support._resolve_mount_base`/`monkeypatch.setitem(_composition.__dict__, "GitWorkspaceProvider", ...)`, no `main.*` references existed. Verified via `rg` — no edit needed.
- [x] 5b.6 `tests/test_unlock.py`: same as 5b.5 — already `_composition`/`_support`-qualified since PR1, no `main.*` references, no edit needed.
- [x] 5b.7 Repoint `tests/test_enterprise_credential.py`: `main._make_provider`/`_make_workspace_provider` were already `_composition.*` (PR1); the 7 `main._make_enterprise_credential_resolver` monkeypatches (all guarding `lock`-path tests) → repointed to `commands.manifest` (bare-imported call site, same rule as onboard in PR5a); the 10 direct (non-monkeypatch) `main._bind_enterprise_source_provider`/`main._bind_enterprise_workspace_provider(...)` calls → repointed to `ec._bind_enterprise_source_provider`/`ec._bind_enterprise_workspace_provider` (the file already imported `enterprise_credential as ec`) since `main` no longer re-exports these symbols after 5b.3 drops the facade. Removed the now-unused `main` import.
- [x] 5b.8 GREEN: `uv run pytest` full suite green — 901 passed, 17 deselected (unchanged from PR5a).
- [x] 5b.9 REFACTOR: `main.py` is 24 lines (well under ~80), containing only the docstring, `app = typer.Typer()`, `@app.callback` `_forge_callback`, the 4-module import, and 4 `register(app)` calls — 0 `@app.command` decorators remain in `main.py`. `commands/manifest.py` is 316 lines (over the ~250 target since it now holds all 5 manifest commands + docstrings; flagged per design's "if not, split" open question — full suite green, byte-identical CLI, so left as a single cohesive module rather than further splitting post-hoc).
- [x] 5b.10 Verify (final): full `CliRunner` suite green, zero assertion changes (901 passed, 17 deselected); `forge --help` lists all 16 commands; `forge lock/project/unlock --help` diffed byte-identical against the PR5a baseline (zero diff, all three); `forge` entry point (`odoo_forge_cli.main:app`) resolves (`import odoo_forge_cli.main` succeeds, `app` importable); `uv run lint-imports` passes (109 files, 351 dependencies, 6/6 contracts kept, `forbidden_modules=["odoo_forge_cli"]` intact); no circular imports (`main` + all 4 `commands/*` + `_composition`/`_support`/`_presentation`/`enterprise_credential` import cleanly together). Adversarial check: retargeting `test_lock.py`'s `commands.manifest` patch to `_composition` raised `AttributeError: <module ... _composition> has no attribute '_make_enterprise_credential_resolver'` (10 errors), then restored — confirms the repoint is load-bearing, not a silent no-op.
