...
- [ ] 1.1 RED: Add failing `tests/manifest/test_lockfile_format.py` cases for version, key order, v2 round-trip, versionless v1, and unknown-version rejection.
- [ ] 1.2 GREEN: Modify `src/odoo_forge/manifest/lockfile.py` with v2 models/dispatch (`git_layers`, `published_layers`, integer versions); REFACTOR canonical JSON and v1 normalization without fabricated published entries.
